...
#
# class Batch(object):
#     def __init__(self, x_s, y_s, x_t_tune, y_t_tune, x_t, batch_size):
#         self.x_s = x_s
#         self.y_s = y_s
...
#         # tf graph input
#         # test阶段需要用训练集数据计算原型，对于目标域，有50条标注数据可用于计算原型
...
#         self.domain_clf_mlp = MLP('domain_output_mlp', [self.FLAGS.n_hidden_s, self.FLAGS.n_domains],
#                                   [identity])
#
...
#         self.probs = tf.nn.softmax(-dists_test, axis=1)
#         self.acc_test = tf.reduce_mean(
#             tf.cast(tf.equal(tf.argmax(self.probs, axis=-1), tf.argmax(self.test_Y, axis=-1)), tf.float32))
#
#         """
...
class Batch(object):
    def __init__(self, x_s, y_s, d_s, x_t_list, y_t_list, d_t_list, x_ts, batch_size):
        """
        参数包括源域标注数据、目标域标注数据（列表分隔，不操作）、目标域未标注数据（按顺序排列，不操作）
        """
        self.x_s = x_s
        self.y_s = y_s
        self.d_s = d_s
        self.x_t_list = x_t_list
        self.y_t_list = y_t_list
        self.d_t_list = d_t_list
        self.x_ts = x_ts
        self.batch_size = batch_size

        self.x_t, self.y_t, self.d_t = self._combine_target_samples()
        self.samples_num = len(self.x_s) + len(self.x_t)
        self.batch_num = int(self.samples_num / self.batch_size)
    ...
    def _combine_target_samples(self):
        x_t, y_t, d_t = [], [], []
        for i in range(len(self.x_t_list)):
            x_t.extend(self.x_t_list[i])
            y_t.extend(self.y_t_list[i])
            d_t.extend(self.d_t_list[i])
        return x_t, y_t, d_t

    def generate(self, shuffle=False):
        x = np.vstack([self.x_s, self.x_t])
        y = np.vstack([self.y_s, self.y_t])
        d = np.vstack([self.d_s, self.d_t])

        if shuffle:
            shuffled_data = np.random.permutation(list(zip(x, y, d)))
        else:
            shuffled_data = list(zip(x, y, d))  # 不shuffle会导致第二轮开始损失为nan，改变batch大小可以解决，估计是某些batch导致损失的值异常。。

        for start in range(self.batch_num):
            yield shuffled_data[start * self.batch_size: min((start + 1) * self.batch_size, self.samples_num)]
...
class AdaptiveModel(object):
    def __init__(self, flags):
        ...
        self.gamma_ds = 0.1
        self.gamma_dp = 0.1
        self.gamma_diff = 1.0  # 正交损失太小了啊！谷歌的实现方案是不是不适用？？？
    ...
    def build_model(self):
        ...
        emb_s = self.shared_encode_mlp.apply(self.X)  # 编码共享表示
        emb_p = self.private_encode_mlp.apply(self.X)  # 编码私有表示，多个域共享私有编码器
        self.emb_s = emb_s
        self.emb_p = emb_p
        self.total_theta = (self.shared_encode_mlp.parameters +
                            self.private_encode_mlp.parameters +
        ...
        loss_p = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=self.D, logits=pred_p))
        # self.D_loss = self.gamma_d * (loss_s + loss_p) + l2_norm
        self.D_loss = self.gamma_ds * loss_s + self.gamma_dp * loss_p + l2_norm
        self.theta_d = self.domain_clf_mlp.parameters
        self.D_solver = tf.train.AdamOptimizer(learning_rate=self.FLAGS.lr).minimize(loss=self.D_loss,
                                                                                     var_list=self.theta_d)
        ...
        self.Diff_loss = diff_loss(emb_s, emb_p, self.gamma_diff)
        self.theta_p = self.private_encode_mlp.parameters
        self.P_loss = self.R_loss + self.gamma_dp * loss_p + self.Diff_loss #+ self.C_loss - l2_norm
        self.P_solver = tf.train.AdamOptimizer(learning_rate=self.FLAGS.lr).minimize(loss=self.P_loss,
                                                                                     var_list=self.theta_p)
        ...
        #                                                                                var_list=self.theta_s)
        self.theta_s = self.shared_encode_mlp.parameters
        self.S_loss = self.R_loss - self.gamma_ds * loss_s + self.Diff_loss + self.C_loss - l2_norm
        self.S_solver = tf.train.AdamOptimizer(learning_rate=self.FLAGS.lr).minimize(loss=self.S_loss,
                                                                                     var_list=self.theta_s)
...
def xavier_init(size):
    ...
    xavier_stddev = 1. / tf.sqrt(in_dim / 2.)  # xavier初始化方法的标准差计算公式，置out_dim=0
    return tf.random_normal(shape=size, stddev=xavier_stddev)
    # return tf.random_normal(shape=size, stddev=xavier_stddev, seed=0)  # 暂时设置随机种子，为了对比效果
...
def diff_loss(shared_feat, task_feat, gamma):
    ...
    # correlation_matrix = tf.matmul(
    #     task_feat, shared_feat, transpose_a=True)
    # cost = tf.sqrt(tf.reduce_sum(tf.square(correlation_matrix))) * gamma
    # cost = tf.where(cost > 0, cost, 0, name='value')
    #
    # assert_op = tf.Assert(tf.is_finite(cost), [cost])
    # with tf.control_dependencies([assert_op]):
